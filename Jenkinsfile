def remote = [:]

pipeline {

    agent any

    environment {
        user = credentials('aclimate_v3_herschel_user')
        host = credentials('aclimate_v3_herschel_host')
        name = credentials('aclimate_v3_herschel_host')
        ssh_key = credentials('aclimate_v3_herschel_key')
    }

    stages {
        stage('Ssh to connect Herschel server') {
            steps {
                script {
                    // Set up remote SSH connection parameters
                    remote.allowAnyHosts = true
                    remote.identityFile = ssh_key
                    remote.user = user
                    remote.name = name
                    remote.host = host
                    
                }
            }
        }
        stage('Update Code') {
            steps {
                script {
                    try {
                        sshCommand remote: remote, command: """
                            cd /var/www/aclimate/aclimate_v3_frontend_sv/
                            git checkout aclimate_salvador
                            git pull origin aclimate_salvador
                        """
                    } catch (Exception e) {
                        echo "Git Pull Error: ${e.message}"
                        error("Failed to update code: ${e.message}")
                    }
                }
            }
        }
        stage('Download latest release') {
            steps {
                script {
                    sshCommand remote: remote, command: """
                        cd /var/www/aclimate/aclimate_v3_frontend_sv/src
                        source /home/aclimate_v3/miniforge3/etc/profile.d/conda.sh
                        conda activate /home/aclimate_v3/miniforge3/envs/aclimate_v3
                        npm install
                        npm run build
                        cd /var/www/aclimate/aclimate_v3_frontend_sv
                        pm2 stop aclimate_v3_sv || pm2 delete aclimate_v3_sv
                        pm2 start ecosystem.config.js
                    """
                }
            }
        }
    }
    
    post {
        failure {
            script {
                echo 'fail :c'
            }
        }

        success {
            script {
                echo 'everything went very well!!'
            }
        }
    }
 
}
